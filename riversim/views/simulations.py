...
from riversim.utils.usgs import *
from riversim.models import *
from riversim.forms.simulations import EditSimulationForm
from riversim.shortcuts import closest_point, render_to_json


def create(request):
    if request.GET.get("polygon", None) is not None:
        if request.user.is_authenticated():
            try:
                ...
                # Return URL
                redirect_url = reverse('simulation_step',
                                       kwargs={'simulation_id': simulation.id, 'step_name': 'aerial_image'})
...
def thumbnail(request, simulation, image, thumbnail_width):
    ...
    force_creation = (request.GET.get("force_creation", None) is not None)
